...
if [ "$CC_SRC_LANGUAGE" = "java" ]; then
	...
	CC_SRC_PATH="../chaincode/cCredential/build/install/cCredential"

	echo Compiling Java code ...
	pushd ../chaincode/cCredential
	./gradlew installDist
	popd
...
packageChaincode() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode package cCredential.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label cCredential_${VERSION} >&log.txt
  res=$?
  set +x
  cat log.txt
...
installChaincode() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode install cCredential.tar.gz >&log.txt
  res=$?
  set +x
  cat log.txt
...
queryInstalled() {
  ...
  set +x
  cat log.txt
	PACKAGE_ID=$(sed -n "/cCredential_${VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" log.txt)
...
queryCommitted() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name cCredential >&log.txt
    res=$?
    set +x
...
chaincodeQuery() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n cCredential -c '{"Args":["queryAllCredentials"]}' >&log.txt
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
