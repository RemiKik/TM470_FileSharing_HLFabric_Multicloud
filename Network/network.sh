...
function printHelp() {
  ...
  echo "      - 'deployCC2' - deploy the cloud credentials chaincode on the channel. Available only in JAVA!!!!"
  echo "      - 'deployCC' - deploy the file sharing contract chaincode on the channel. Available only in JAVA!!!!!"
  ...
  echo " To bring network up, generate artifacts, create and join channel use:"
  echo "  network.sh up createChannel -ca -c mychannel -s couchdb "
  echo " To deploy Cloud Credentials contract use: "
  echo "  network.sh deployCC -l java"
  echo " To deploy File Sharing contract use: "
  echo "  network.sh deployCC2 -l java"
...
## Call the script to isntall and instantiate second chaincode on the channel
function deployCC2() {

  scripts/deployCC2.sh $CHANNEL_NAME $CC_SRC_LANGUAGE $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
